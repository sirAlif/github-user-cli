...
describe('API Endpoints', () => {
  ...
  describe('POST /user', () => {
    it('should add a user successfully', async () => {
      ...
      const response = await request(app)
        .post('/user').send({ username: 'octocat' });
  ...
  describe('PUT /user', () => {
    it('should update a user successfully', async () => {
      mockAddUserCommand.mockResolvedValue({ error: null });
      const response = await request(app)
        .put('/user').send({ username: 'octocat' });
      expect(response.status).toBe(200);
  ...
  describe('DELETE /user/:username', () => {
    it('should delete a user successfully', async () => {
      ...
      const response = await request(app).delete('/user/octocat');
      expect(response.status).toBe(200);
  ...
  describe('GET /user/:username', () => {
    it('should get user details successfully', async () => {
      ...
      const response = await request(app).get('/user/octocat');
      expect(response.status).toBe(200);
  ...
  describe('POST /populate', () => {
    it('should populate users successfully', async () => {
      ...
      const response = await request(app).post('/populate');
      expect(response.status).toBe(200);
  ...
  describe('POST /ai/text', () => {
    it('should execute AI command successfully', async () => {
      ...
      const response = await request(app).post('/ai/text')
    ...
    it('should return 400 for missing text', async () => {
      const response = await request(app).post('/ai/text').send({});
      expect(response.status).toBe(400);
    ...
    it('should return 500 if aiCommand fails', async () => {
      ...
      const response = await request(app)
        .post('/ai/text').send({ text: 'Get users' });
      expect(response.status).toBe(500);
      expect(response.body.error)
  ...
  describe('POST /ai/voice', () => {
    it('should execute AI voice command successfully', async () => {
      ...
      const response = await request(app)
        .post('/ai/voice').attach('file', Buffer.from('audio'), 'audio.wav');
      expect(response.status).toBe(200);
      expect(response.body.data[0].username).toBe('octocat');
    });
    ...
    it('should return 500 if aiCommand fails', async () => {
      ...
      const response = await request(app).post('/ai/voice')
